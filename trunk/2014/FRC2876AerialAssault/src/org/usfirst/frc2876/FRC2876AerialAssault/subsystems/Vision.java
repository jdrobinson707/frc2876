...
public class Vision extends Subsystem {
    ...
    //4 inch wide, 1 foot 11.5 inch tall -> horizontal
    //4 inch wide, 2 foot 8 inches -> vertical
    ...
    public int findTargets() {
        ...
        if (now - last > 1) {
            ...
            try {
                ...
                BinaryImage thresholdImage = image.thresholdHSV(79, 255, 71, 255, 10, 255);   // keep only green objects
			...
			target.verticalIndex = verticalTargets[0];
                        //We are first looping through vertical targets for the vertical tape, if there is no vertical tape,
                        //then there is no reason to go through the horizontal loop 
			for (int i = 0; i < verticalTargetCount; i++)
			{
				ParticleAnalysisReport verticalReport = filteredImage.getParticleAnalysisReport(verticalTargets[i]);
				int turnDirection = getTurnDirection(verticalReport.center_mass_x);
                                System.out.println("turnDirection: " + turnDirection);
                                
                                //WARNING HERE IS A RETURN STATEMENT
                                if (turnDirection != 0)
                ...
                //(1/25/11) following 8 lines were moved out of the "If filterimage if statement"
                if(target.Hot)
                    {
                            System.out.println("Hot target located");
                           // System.out.println("Distance: " + distance);     //commented out (1/25/11)
                    } else {
	...
	boolean hotOrNot(TargetReport target)
	{
        ...
        int getTurnDirection(int cmX) {
            int turnDirection = 0;
            if (cmX < Y_IMAGE_RES / 2- 50) { 
                turnDirection = -1;
            } else if (cmX > Y_IMAGE_RES / 2 + 50) {
                turnDirection = 1;   
            }
            return turnDirection;
        }
